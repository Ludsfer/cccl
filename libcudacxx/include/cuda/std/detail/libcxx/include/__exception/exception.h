...
#if defined(_LIBCUDACXX_ABI_VCRUNTIME) && (!defined(_HAS_EXCEPTIONS) || _HAS_EXCEPTIONS != 0) && !defined(__cuda_std__)
...
public:
  ...
  _CCCL_HOST_DEVICE virtual ~exception() noexcept;
  _CCCL_HOST_DEVICE virtual const char* what() const noexcept;
};
...
public:
  ...
  _CCCL_HOST_DEVICE virtual ~bad_exception() noexcept;
  _CCCL_HOST_DEVICE virtual const char* what() const noexcept;
};
