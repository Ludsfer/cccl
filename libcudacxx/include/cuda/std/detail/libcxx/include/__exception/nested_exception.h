...
public:
  ...
  _CCCL_HOST_DEVICE virtual ~nested_exception() noexcept;

  // access functions
  _LIBCUDACXX_NORETURN _CCCL_HOST_DEVICE void rethrow_nested() const;
...
#if defined(_CCCL_COMPILER_MSVC) // The operator 'dynamic_cast' is not allowed in device code
    : false_type {};
...
void rethrow_if_nested(const _Ep& __e) {
...
template <class _Ep, __enable_if_t<!__can_dynamic_cast<_Ep, nested_exception>::value, int> = 0>
inline _LIBCUDACXX_HIDE_FROM_ABI _LIBCUDACXX_INLINE_VISIBILITY
void rethrow_if_nested(const _Ep&) {}
